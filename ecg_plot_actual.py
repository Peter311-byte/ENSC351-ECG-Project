...
BPM_HISTORY_SIZE = 12  # Increased for better smoothing
bpm_history = deque(maxlen=BPM_HISTORY_SIZE)
recent_rr_intervals = deque(maxlen=20)  # Store more RR intervals
...
SAMPLE_COUNTER = 0
CURRENT_BPM = 0.0
all_peak_times = deque(maxlen=50)
last_bpm_update_time = 0  # Track when we last updated BPM
...
def update():
    ...
    global recent_rr_intervals, last_bpm_update_time
    ...
    if SAMPLE_COUNTER >= N // 2:
        ...
        if len(peaks) >= 2:
            # IMPROVED BPM CALCULATION using RR intervals
            
            # Convert peak indices to absolute times
            current_time = SAMPLE_COUNTER / FS
            peak_absolute_times = []
            for peak_idx in peaks:
                time_offset = (N - peak_idx) / FS
                peak_time = current_time - time_offset
                peak_absolute_times.append(peak_time)
            
            # Update peak history
            for pt in peak_absolute_times:
                if len(all_peak_times) == 0 or pt > all_peak_times[-1]:
                    all_peak_times.append(pt)
            
            # Calculate RR intervals from ALL recent peaks in history
            if len(all_peak_times) >= 3:
                # Use last 20 peaks for RR interval calculation
                recent_peaks = list(all_peak_times)[-20:]
                rr_intervals = np.diff(recent_peaks)
                
                # Filter for realistic heart rates (40-120 BPM)
                # 40 BPM = 1.5s, 120 BPM = 0.5s
                valid_rr = rr_intervals[(rr_intervals >= 0.5) & (rr_intervals <= 1.5)]
                
                if len(valid_rr) >= 2:
                    # Add valid RR intervals to history
                    for rr in valid_rr[-5:]:  # Add last 5 valid intervals
                        recent_rr_intervals.append(rr)
                    
                    # Calculate BPM from median RR interval
                    if len(recent_rr_intervals) >= 5:
                        # Use median for robustness (handles outliers)
                        median_rr = np.median(recent_rr_intervals)
                        ...
                        if 40 < new_bpm < 120:
                            ...
                            # Update display BPM (use weighted average for smoothness)
                            if len(bpm_history) >= 5:
                                # Use exponentially weighted average of recent BPMs
                                weights = np.exp(np.linspace(-1, 0, len(bpm_history)))
                                weights = weights / np.sum(weights)
                                CURRENT_BPM = np.average(list(bpm_history), weights=weights)
                                
                                bpm_label.setText(
                                    f"<span style='color: #FF69B4; font-size: 30pt; font-weight: bold;'>BPM: {CURRENT_BPM:.1f}</span>"
                                )
            
            # Also show instant BPM for reference
            instant_bpm = (len(peaks) / WINDOW_SEC) * 60 if len(peaks) > 0 else 0
            info_text = f"Peaks: {len(peaks)} in {WINDOW_SEC}s | Instant: {instant_bpm:.1f} | Smoothed BPM: {CURRENT_BPM:.1f}"
